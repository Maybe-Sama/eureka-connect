...
import { StudentColorCard } from './StudentColorCard'
...
interface ColorConfigPanelProps {
  ...
  onClose: () => void
  students: Student[]
  onColorChange: (studentId: number, color: string) => void
}
...
export const ColorConfigPanel = ({ isOpen, onClose, students, onColorChange }: ColorConfigPanelProps) => {
  const [studentColors, setStudentColors] = useState<Record<number, string>>({})
  const [hasChanges, setHasChanges] = useState(false)
  const [colorPickerOpen, setColorPickerOpen] = useState(false)
  const [selectedStudent, setSelectedStudent] = useState<Student | null>(null)

  // Cargar colores guardados del localStorage al abrir el panel
  useEffect(() => {
    if (isOpen) {
  ...
  // Función para obtener el color actual de un alumno
  const getCurrentColor = (studentId: number) => {
    return studentColors[studentId] || PREDEFINED_COLORS[studentId % PREDEFINED_COLORS.length].value
  }

  // Función para cambiar el color de un alumno
  ...
  // Función para abrir el picker de colores
  const handleColorClick = (student: Student) => {
    setSelectedStudent(student)
    setColorPickerOpen(true)
  }

  // Función para seleccionar un color del picker
  const handleColorSelect = (color: string) => {
    if (selectedStudent) {
      handleColorChange(selectedStudent.id, color)
    }
  ...
  const handleSave = () => {
    ...
    // Aplicar los cambios a todos los alumnos
    Object.entries(studentColors).forEach(([studentId, color]) => {
      onColorChange(Number(studentId), color)
    })
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-background rounded-xl shadow-2xl w-full max-w-2xl flex flex-col p-4"
        ...
        >
          ...
          <div className="flex items-center justify-between pb-4 border-b border-border">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-xl font-semibold text-foreground">Configuración de Colores</h2>
                <p className="text-sm text-foreground-muted">Asigna colores únicos a cada alumno</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="py-4 overflow-y-auto flex-1">
              ...
              {students && students.length > 0 ? (
                students.map((student) => (
                  <StudentColorCard
                    key={student.id}
                    student={student}
                    currentColor={getCurrentColor(student.id)}
                    onColorClick={() => handleColorClick(student)}
                  />
                ))
              ) : (
                <div className="text-center py-8">
                  <p className="text-foreground-muted">No hay alumnos disponibles</p>
                </div>
              )}
            </div>
            ...
            <div className="mt-6 p-4 bg-background-tertiary rounded-lg">
              <p className="text-sm text-foreground-muted text-center">
                💡 Haz clic en el cuadrado de color de cada alumno para abrir la paleta de colores
              </p>
            </div>
          </div>

          {/* Footer */}
          <div className="flex items-center justify-between pt-4 border-t border-border bg-background-secondary">
            <button
              onClick={handleReset}
              className="flex items-center space-x-2 px-4 py-2 text-sm text-foreground-muted hover:text-foreground transition-colors"
            >
      ...
      <ColorPickerModal
        ...
        currentColor={selectedStudent ? getCurrentColor(selectedStudent.id) : ''}
